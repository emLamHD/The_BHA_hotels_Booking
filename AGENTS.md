...
> Shared contract: the implementer selected by the Master Execution Prompt writes, the paired reviewer reviews read-only.
>
...
- `Front_End/Admin_Web/` — imported TailAdmin template baseline (PR #30), plus the PMS Reservation Board frontend and a front-desk reservation-creation workspace added by `ADMIN-002.1` (PR #32): room/date timeline, multi-property switching, assigned/unassigned reservations, operational blocks, and reservation lifecycle/folio/move demonstrations. `PMS-CAL-001.1` (PR #41) connected the **board** to the real backend: it reads the live Reservation Board projection from `Back_End/` over HTTPS. Everything else in this application — the front-desk creation workspace and the lifecycle/folio/move demonstrations — is still driven by deterministic local mock state. `PMS-CAL-001.2` opened the first Admin **write** path in `Back_End/`: CP01 (PR #43) added the local write gate, and CP02 added the first endpoint behind it — `POST /api/admin/v1/properties/{propertyId}/reservation-assignments`, which creates one ReservationAssignment segment and is reachable only from a local Development host that explicitly opts in. `PMS-CAL-001.2-CP03A` makes the Admin board its first caller: an unassigned range on the board can be assigned to an Active room of the same sold RoomType, and the board then re-reads the backend. `PMS-CAL-001.2-CP03B` extends that same dialog to a controlled cross-RoomType placement — an Active room of a different RoomType, gated by an explicit operational confirmation and a recorded reason, through the same `reservation-assignments` endpoint and contract. `PMS-CAL-001.2-CP04B` adds two more narrow routes behind the same gate — one-segment `move` and `unassign` on an existing assignment segment — which `PMS-CAL-001.2-CP04D` wires into the board's stay popover. `PMS-CAL-001.3-CP01` opens the first operational-block write path: `POST /api/admin/v1/properties/{propertyId}/operational-blocks` creates exactly one OperationalBlock segment through the same local gate, and `PMS-CAL-001.3-CP02` adds `POST .../operational-blocks/{segmentId}/cancel`, which cancels exactly one Effective block segment. `PMS-CAL-001.3-CP03` makes the board the create route's first caller: a toolbar dialog blocks one Active room for a confirmed range of nights within the visible board, then re-reads the backend. `PMS-CAL-001.3-CP04` makes it the cancel route's first caller too: an operational-block bar opens a details popover whose `Cancel block` action confirms the segment's own full nights and `expectedVersion`, sends exactly one cancel, and lets the block disappear only from the re-read. Assignment split/swap/batch, operational-block move/split, Admin authentication/RBAC and real OTA behavior still do not exist over HTTP. The locked target architecture remains one shared `Back_End/` solution serving both `Front_End/Customer_Web/` and `Front_End/Admin_Web/`.
...
`Owner + Control Tower → Operations Coordinator → ACTIVE_EXECUTOR (implementer) → Owner → OC review → Owner decision`

Owner Hồ Đình Lâm alone decides Ready/merge, branch cleanup and whether the next task starts.

Each Master Execution Prompt selects exactly one of two valid role pairs (`docs/governance/RULES.md` §2.4):

- `IMPLEMENTER: CLAUDE` + `REVIEWER: CODEX_READ_ONLY`
- `IMPLEMENTER: CODEX` + `REVIEWER: CLAUDE_READ_ONLY`

Combined values (e.g. `CLAUDE/CODEX`) are never valid. The selected implementer is the sole writable `ACTIVE_EXECUTOR` for that work item; the paired reviewer is read-only. The pair does not change during the work item or any of its corrections.

There are two distinct activation contexts.

### 2.A Implementation context

- `ACTIVE_EXECUTOR` — the agent `IMPLEMENTER` names in the current Master Execution Prompt — is the only write-capable implementer for this repository, for that work item.
- `ACTIVE_EXECUTOR` requires a valid Master Execution Prompt containing at minimum `IMPLEMENTER`, `REVIEWER`, `REPOSITORY`, `FEATURE_BRANCH`, baseline, scope, acceptance, checks, skill policy and stop conditions before making any edit.
...
- If the Master Execution Prompt is missing a required field, or another coding agent may still have write access, `ACTIVE_EXECUTOR` returns `BLOCKED`.

### 2.B Dedicated review context
...
- Only Owner may invoke the review. `ACTIVE_EXECUTOR` must never run or trigger it. At the required handoff (see §13), `ACTIVE_EXECUTOR` must explicitly instruct Owner to invoke the review — announcing that required Owner action is permitted and is not equivalent to `ACTIVE_EXECUTOR` invoking it.

## 3. Write lock

- `ACTIVE_EXECUTOR` is the only coding agent with write access to the repository checkout, for every work item, at every phase.
...
- Before Owner invokes review, `ACTIVE_EXECUTOR` stops all writes and leaves the working tree at a stable, reviewable checkpoint for the duration of the review.
- A correction cycle requires an OC correction prompt and Owner activation before `ACTIVE_EXECUTOR` resumes writing; the role pair does not change.
...
1. Confirm `WORK_ITEM`, `IMPLEMENTER`, `REVIEWER`, `REPOSITORY`, `FEATURE_BRANCH`, baseline, scope and acceptance from the Master Execution Prompt.
...
- This single-checkout rule does not grant the reviewer, a subagent, or a parallel implementation write access — the roles in §2–§3 are unchanged.
...
- A skill never overrides RULES, scope, the selected implementer/reviewer roles or test requirements.
