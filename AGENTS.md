...
- Graphify (`graphifyy==0.9.48`) has completed a **workspace-local, project-scoped**, code-only tooling-adoption pilot (`TOOL-GRAPHIFY-001`), gate-closed by a **Claude-run governance replay** (`TOOL-GRAPHIFY-001-DOCS-CLOSEOUT-C4`). Owner's original hands-on install/config/dry-run/pilot is retained as historical, preliminary technical evidence only — it was not itself sufficient to close the adoption gate under the single-writer invariant (§3, §2.A), because Owner, not Claude, performed those filesystem writes. Claude subsequently replayed the full pilot as sole writable implementer (isolating and backing up the prior artifacts, verifying the exact CLI version, running the project-scoped install, recreating the code-only graph and clustering, running and independently source-verifying the pilot query, and confirming graph reuse from a separate read-only Claude Code session in the same workspace) before the gate was marked closed. "Project-scoped" describes where Claude Code discovers and applies the skill in that workspace — it does **not** mean repository-tracked. The skill (`.claude/skills/graphify/SKILL.md` and its version sidecar) and its graph outputs (`graphify-out/`) are excluded from Git via `.git/info/exclude`; `git ls-files` returns no match for either path. A fresh clone, a different worktree, or another machine will **not** automatically have Graphify installed.
- Graphify invocation always requires **both** conditions: (1) it is available and sufficiently current in the active workspace, and (2) the current Master Execution Prompt explicitly authorizes it through a valid `GRAPHIFY_POLICY`. Availability, task relevance, or Claude's own model-selected judgment that a task "looks like" architecture/dependency/relationship/impact-analysis work never grant authority by themselves — only prompt-listed/approved skills may be used (unchanged from the general skill rule above), and Graphify is no exception.
- Every Master Execution Prompt's skill policy must include an explicit `GRAPHIFY_POLICY` field:
  ...
  - `REQUIRED_FOR_PREFLIGHT_IMPACT_ANALYSIS`: Claude must use Graphify for the prompt-defined preflight impact analysis. Claude first verifies workspace availability and graph freshness (`git ls-files` returning nothing for the skill/graph paths is not evidence of absence by itself — check local existence, e.g. `.claude/skills/graphify/SKILL.md`, directly). If Graphify is absent or unusably stale and installation/rebuild is not separately authorized, Claude follows the prompt's declared `GRAPHIFY_UNAVAILABLE_OR_STALE` action; if none is supplied, Claude returns `BLOCKED` rather than silently installing, rebuilding, or skipping a required capability.
  - `ALLOWED_IF_RELEVANT`: Graphify is authorized but optional. Claude may automatically invoke it — this is the **only** policy value under which model-selected auto-invocation is permitted — only when it is available in the current workspace, the graph is sufficiently current for the question, and the task concerns architecture/dependency/relationship/impact analysis. If unavailable or stale, Claude continues with source/tests and reports the limitation, unless the prompt states otherwise. Installation/rebuild still requires separate explicit authorization.
  - `NOT_APPLICABLE`: Claude must not invoke Graphify at all — no inspection, query, install, update or rebuild for that work item.
