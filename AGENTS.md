...
- `Front_End/Admin_Web/` — imported TailAdmin template baseline (PR #30), plus the PMS Reservation Board frontend and a front-desk reservation-creation workspace added by `ADMIN-002.1` (PR #32): room/date timeline, multi-property switching, assigned/unassigned reservations, operational blocks, and reservation lifecycle/folio/move demonstrations. `PMS-CAL-001.1` (PR #41) connected the **board** to the real backend: it reads the live Reservation Board projection from `Back_End/` over HTTPS. Everything else in this application — the front-desk creation workspace and the lifecycle/folio/move demonstrations — is still driven by deterministic local mock state. `PMS-CAL-001.2` opened the first Admin **write** path in `Back_End/`: CP01 (PR #43) added the local write gate, and CP02 adds exactly one endpoint behind it — `POST /api/admin/v1/properties/{propertyId}/reservation-assignments`, which creates one ReservationAssignment segment and is reachable only from a local Development host that explicitly opts in. No Admin frontend calls it yet, and assignment move/unassign/split, operational-block mutation, Admin authentication/RBAC and real OTA behavior still do not exist over HTTP. The locked target architecture remains one shared `Back_End/` solution serving both `Front_End/Customer_Web/` and `Front_End/Admin_Web/`.
