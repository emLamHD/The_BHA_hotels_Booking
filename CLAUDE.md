...
- Claude requires the current Master Execution Prompt (`WORK_ITEM`, `IMPLEMENTER: CLAUDE`, `REVIEWER: CODEX_READ_ONLY`, `REPOSITORY`, `FEATURE_BRANCH`, `WORKING_TREE_MODE` — defaults to `PRIMARY_CHECKOUT_ONLY` if omitted — `LINKED_WORKTREE` — defaults to `NOT_AUTHORIZED`, see `AGENTS.md` §8 / `docs/governance/RULES.md` §5 for the exception contract — baseline, scope, acceptance, checks, skill policy, stop conditions) before making any edit.
